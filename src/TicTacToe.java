...
public class TicTacToe extends Canvas {
    ...
    public static boolean winInDiagonalBS(int[][] board, int piece) {
        ...
        int pieceCounter = 0;
        // number of rows with potentially winning positions
        int totalRows = rowsIn(board) - WIN_LENGTH;
        // number of columns with potentially winning positions
        int totalColumns = columnsIn(board) - WIN_LENGTH;

        // iterating through the columns (particularly the potential winning positions)
        for (int currentColumn = 0; currentColumn <= totalColumns; currentColumn++) {
            //iterating through the rows (particularly the potential winning positions)
            for (int currentRow = 0; currentRow <= totalRows; currentRow++) {
                // traversing through the current diagonal backslash row
                for (int i = currentColumn, j = currentRow; (i < columnsIn(board)) && (j < currentRow + 3); i++, j++) {
                    if (board[j][i] == piece) {
                        pieceCounter++;
                    }
                }
                // conditional checks if 3 pieces occurred in a row is a win; returns true
                if (pieceCounter >= 3) {
                    return true;
                }// since less than 3 pieces in row, we can reset the count for pieces (as we move to new row)
                else {
                    pieceCounter = 0;
                }
            }
    ...
    public static boolean winInDiagonalFS(int[][] board, int piece) {
        ...
        // iterating through the columns (particularly the potential winning positions)
        for (int currentColumn = columnsIn(board) - 1; currentColumn >= totalColumns; currentColumn--) {
            //iterating through the rows (particularly the potential winning positions)
            for (int currentRow = 0; currentRow <= totalRows; currentRow++) {
                // traversing through the current diagonal backslash row
                for (int i = currentColumn, j = currentRow; (i >= 0 && (j < currentRow + 3)); i--, j++) {
                    if (board[j][i] == piece) {
                        pieceCounter++;
                    }
                }
    ...
    public static int[] hint(int[][] board, int piece) {
        ...
        // returning default values within 1D array (when no winning moves)
        return new int[]{-1, -1};
    }
